...
# Function to Load Inboxes
def load_inboxes():
    if not os.path.exists(INBOX_DB):
        return {}
    with open(INBOX_DB, "r") as file:
        return json.load(file)

# Function to Save Inboxes
def save_inboxes(inboxes):
    with open(INBOX_DB, "w") as file:
        json.dump(inboxes, file, indent=4)
...
def handle_client(secure_socket, address):
    try:
        ...
        print(f"[Bastion] User '{username}' has connected.")
        logging.info(f"User '{username}' connected from {address}")

        # Ensuring the user has an inbox
        ...
        while True:
            ...
            print(f"[Bastion] Decrypted Message from {username}:", decrypted_message)
            logging.info(f"Message from {username}: {decrypted_message}")

            # Storing the message in the user's inbox
            inboxes[username].append(decrypted_message)
            save_inboxes(inboxes)

            # Sending a confirmation response to the client
            response = f"Message received and stored in your inbox."
...
def start_bastion_server():
    ...
    Listens for multiple client connections, receives and logs encrypted messages, and stores them in user-specific inboxes.
    """
